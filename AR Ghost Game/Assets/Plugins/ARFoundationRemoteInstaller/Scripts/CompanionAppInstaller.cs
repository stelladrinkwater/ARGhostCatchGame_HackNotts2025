...
namespace ARFoundationRemote.Editor {
    internal class CompanionAppInstaller: IPreprocessBuildWithReport, IPostprocessBuildWithReport {
        ...
        static void build(BuildOptions options, InstallerSettings settings) {
            ...
            // Unity 6 build fails when ARCompanion.unity is in Resources folder
            const string oldPath = "Assets/Plugins/ARFoundationRemoteInstaller/Resources/ARCompanion.unity";
            const string newPath = "Assets/Plugins/ARFoundationRemoteInstaller/ARCompanion.unity";
            ...
            Assert.IsTrue(File.Exists(newPath), $"AR Foundation Remote: scene not found at path '{newPath}'. Please reimport the plugin form Asset Store.");

            string[] scenes = { newPath };
            try {
